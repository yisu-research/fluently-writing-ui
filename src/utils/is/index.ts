...
export function isEmail<T extends string>(value: T | unknown): value is T {
  return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value as string)
}

export function isPassword<T extends string>(value: T | unknown): value is T {
  return /^(?!([^(0-9a-zA-Z)])+$).{8,32}$/.test(value as string)
}

export function isCode<T extends string>(value: T | unknown): value is T {
  return /^\d{6}$/.test(value as string)
}
